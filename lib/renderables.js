...
export async function renderableBundle(bundleModule) {
  ...
  // by convention, `elements` are parts that are attached outside of render
  // we pass in a never-resolving promise to leverage until() for getter
  // promises
  data.elements = new Proxy({}, {get: () => new Promise(() => {})});
