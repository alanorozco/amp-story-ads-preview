...
export async function sendRenderable(response, {head, body}) {
  ...
  const rendered = Scaffold(context, {
    head: await head(context),
    body: await body(context),
  });
...
export function sendBundle(response, module, {css}) {
  const [name] = Object.keys(module);
  const loadedModule = module[name];
  const dist = `dist/${name}`;
  const head = async context => {
    const src = `${dist}.js`;
    const head = [Script(context, {src})];
    if (css) {
      const src = `${dist}.css`;
  ...
  const body = async context =>
    loadedModule.render(context, await loadedModule.data());
  sendRenderable(response, {head, body});
}
